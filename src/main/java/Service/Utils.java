...
public class Utils {
	...
	public static whiteboard.Whiteboard.point convertToPointProto(Point2D point) {
		return pointCache.computeIfAbsent(point,
				p -> whiteboard.Whiteboard.point.newBuilder().setX(p.getX()).setY(p.getY()).build());
	}
	...
	public static Whiteboard._CanvasShape shape2ProtoShape(WBSYS.CanvasShape canvasShape) {
		...
		return Whiteboard._CanvasShape.newBuilder()
			...
			.addX(canvasShape.getX1())
			.addX(canvasShape.getX2())
			.addX(canvasShape.getY1())
			.addX(canvasShape.getY2())
			.setText(canvasShape.getText() == null ? "" : canvasShape.getText())
			.setFill(canvasShape.isFill())
			.setUsername(canvasShape.getUsername())
			.addAllPoints(points == null ? List.of() : points.stream().map(Utils::convertToPointProto).toList())
	...
	static CanvasShape protoShape2Shape(Whiteboard._CanvasShape _canvasShape) {
		...
		if (_canvasShape.getShapeString().equals("pen") || _canvasShape.getShapeString().equals("eraser")) {
			shape = new CanvasShape(_canvasShape.getShapeString(), new Color(Integer.parseInt(_canvasShape.getColor())),
					_canvasShape.getUsername(), protoPointsToArrayList(_canvasShape.getPointsList().stream().toList()),
					_canvasShape.getStrokeInt());
		}
		else if (_canvasShape.getShapeString().equals("text")) {
			shape = new CanvasShape(_canvasShape.getShapeString(), new Color(Integer.parseInt(_canvasShape.getColor())),
					_canvasShape.getX(0), _canvasShape.getX(1), _canvasShape.getX(2), _canvasShape.getX(3),
					_canvasShape.getText(), _canvasShape.getFill(), _canvasShape.getUsername(),
					_canvasShape.getStrokeInt());
		}
		else {
			shape = new CanvasShape(_canvasShape.getShapeString(), new Color(Integer.parseInt(_canvasShape.getColor())),
