...
public class WhiteBoardServiceImpl extends WhiteBoardServiceGrpc.WhiteBoardServiceImplBase {
	...
	public void checkPeerName(com.google.protobuf.StringValue request, StreamObserver<Response> responseObserver) {
		...
		if (wb.userAgents.containsKey(request.getValue())) {
			responseObserver
				.onNext(Response.newBuilder().setSuccess(false).setMessage("Username Already exists").build());
		}
		else {
	...
	public void getApprove(com.google.protobuf.StringValue request, StreamObserver<Response> responseObserver) {
		...
		if (wb.getApproveFromUI(request.getValue())) {
			responseObserver
				.onNext(Response.newBuilder().setSuccess(true).setMessage("Welcome " + request.getValue()).build());
			// chatStreamObservers.put(request.getValue(),
		}
		else {
			responseObserver.onNext(
					Response.newBuilder().setSuccess(false).setMessage("You have been rejected by manager").build());
		}
		responseObserver.onCompleted();
	}

	@Override
	public void registerPeer(Whiteboard.IP_Port ip_port, io.grpc.stub.StreamObserver<Response> responseObserver) {
		...
		// java.util.concurrent.TimeUnit.SECONDS); // 关闭旧的 channel
		// channel = ManagedChannelBuilder.forAddress(ip_port.getIp(),
		// Integer.parseInt(ip_port.getPort()))
		...
		// try {
		// InputStream inputStream =
		// getClass().getClassLoader().getResourceAsStream("grpc_service_config.json");
		// byte[] bytes = inputStream.readAllBytes();
		// grpc_service_config = Arrays.toString(bytes);
		// } catch (IOException e) {
		// throw new RuntimeException(e);
		// }
		// if (grpc_service_config == null || grpc_service_config.isEmpty() ||
		// grpc_service_config.isBlank()) {
		// logger.severe("grpc_service_config.json not found");
		// channel = ManagedChannelBuilder.forAddress(ip_port.getIp(),
		// Integer.parseInt(ip_port.getPort())).usePlaintext().build();
		// }else{
		// channel = ManagedChannelBuilder.forAddress(ip_port.getIp(),
		// Integer.parseInt(ip_port.getPort())).
		...
		Context.current().fork().run(() -> {
			for (String editingUser : wb.getEditingUser()) {
				wb.userAgents.get(ip_port.getUsername())
					.updEditing(Whiteboard.SynchronizeUserRequest.newBuilder()
						.setOperation("add")
						.setUsername(editingUser)
		...
		// String jwtToken =
		// Jwts.builder().setSubject(ip_port.getUsername()).signWith(Constants.JWT_KEY).compact();
	...
	@Override
	public synchronized void pushShape(Whiteboard._CanvasShape requestShape,
			StreamObserver<com.google.protobuf.Empty> responseObserver) {
		logger.severe("Received shape: " + requestShape.getShapeString());
		CanvasShape shape = protoShape2Shape(requestShape);

		new CanvasShape(requestShape.getShapeString(), new Color(Integer.parseInt(requestShape.getColor())),
				requestShape.getX(0), requestShape.getX(1), requestShape.getX(2), requestShape.getX(3),
				requestShape.getStrokeInt());
	...
	// public io.grpc.stub.StreamObserver<whiteboard.Whiteboard._CanvasShape>
	// sPushShapeBlockHandler(
	...
	public io.grpc.stub.StreamObserver<whiteboard.Whiteboard._CanvasShape> sPushShape(
		...
		return new io.grpc.stub.StreamObserver<whiteboard.Whiteboard._CanvasShape>() {
				...
				if (wb.checkConflictOk(shape)) {
					...
					if (WBServer.FCOFF) {
						...
						try {
							...
							CompletableFuture.runAsync(() -> {
								// 在异步回调中进行上下文变换，通过 AsyncEntry 的 getAsyncContext 方法获取异步
								// Context
					...
					if (isOk) {
						responseObserver.onNext(Response.newBuilder()
							.setSuccess(false)
							.setMessage("Conflict with other editing")
							.build());
					}
					isOk = false;
			...
			public void onCompleted() {
				...
				responseObserver.onCompleted();// 没有这句则绘制不落实 待调研onNext里的responseObserver
												// onnext是否会自动调用onCompleted
			}
