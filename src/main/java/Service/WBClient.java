	...
	public static void main(String[] args) {
		if (args.length == 3 || args.length == 4) {
			if (!isValidPort(args[1])) {
				...
				com.google.protobuf.StringValue checkPeerNameRequest = StringValue.newBuilder()
					.setValue(username)
					.build();
								...
								public void onNext(Response response) {
									if (response.getSuccess()) {
										...
										if (ServerStub == null) {
											...
											logger
												.info("Successfully get stub, channel is connected." + client.channel);
										...
										if (serverSecuredStub == null) {
											...
											logger.info("Successfully get secured stub, channel is connected."
													+ client.channel);
				...
				try {
					ServerStub.withDeadlineAfter(5, TimeUnit.SECONDS)
						.checkPeerName(checkPeerNameRequest, checkPeerNameResponseObserver);
