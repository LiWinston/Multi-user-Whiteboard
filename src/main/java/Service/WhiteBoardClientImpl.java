...
public class WhiteBoardClientImpl extends WhiteBoardClientServiceGrpc.WhiteBoardClientServiceImplBase {
	...
	public void updatePeerList(whiteboard.Whiteboard.UserList request,
		...
		responseObserver.onNext(whiteboard.Whiteboard.Response.newBuilder()
			.setSuccess(true)
			.setMessage("Successfully upd usrlst")
			.build());
		responseObserver.onCompleted();
	}
	...
	@Override
	public void sPreviewShapes(Whiteboard._CanvasShape request, StreamObserver<Empty> responseObserver) {
	...
	public void updateShapeList(whiteboard.Whiteboard._CanvasShapeList request,
		...
		if (wb.getLocalShapeQ().size() != request.getShapesCount()) {
			responseObserver.onNext(whiteboard.Whiteboard.Response.newBuilder()
				.setSuccess(false)
				.setMessage("Shape list size not match")
				.build());
		}
		else {
			responseObserver.onNext(whiteboard.Whiteboard.Response.newBuilder()
				.setSuccess(true)
				.setMessage("Successfully upd shapelist")
				.build());
		}
		responseObserver.onCompleted();
	}
