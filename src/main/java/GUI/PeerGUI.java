...
public class PeerGUI implements IClient, MouseListener, MouseMotionListener, ActionListener, WindowListener {
						...
						if (currentShapeType.equals("pen") || currentShapeType.equals("eraser")) {
							...
							try {
								String texts = JOptionPane
									.showInputDialog(peerFrame, "input your text", "text", JOptionPane.PLAIN_MESSAGE,
											null, null, null)
									.toString();
								// if (texts.isEmpty()) {
								// wb.requestForceClearTmp();
	...
	public void reDraw() {
		SwingUtilities.invokeLater(() -> {
			...
			new Thread(() -> {
				...
				// AtomicReference<ArrayList<CanvasShape>> shapeArrayList = new
				// AtomicReference<>(wb.getCanvasShapeArrayList());
	...
	public void loseFocus() {
		...
		if (wb.getTempShapes().containsKey(username)) {
			// JOptionPane.showMessageDialog(peerFrame, "Lose focus, your preview will be
			// canceled on your next action.");
			wb.requestForceClearTmp();
		}
